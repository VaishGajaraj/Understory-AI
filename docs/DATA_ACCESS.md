...
**Run in-region.** The archive is hundreds of terabytes; the pipeline is designed for direct S3 access from a VM in us-west-2, streaming only the coherence layer out of each granule. A download-based workflow punishes every experiment — use it only for spot checks. v0 retrieves full granules into a local verified granule cache (keyed by granule id, size/MD5-checked against the catalog) (`uv run understory build-stack ...`); S3 range reads remain the production target.
